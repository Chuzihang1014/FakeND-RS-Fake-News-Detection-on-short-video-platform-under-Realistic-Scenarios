...
from models.FakeND_RS import FakeND_RSModel
...
def FakeND_RS_collate_fn(batch):
    num_comments = 23
    num_frames = 83
    num_audioframes = 50
    ...
    for idx in range(len(comments_like)):
        ...
        if comments_like_one.shape != torch.Size([0]):
            sorted_tuples = sorted(zip(comments_inputid_one, comments_mask_one, comments_like_one), key=lambda s: s[2],reverse=True)
            if sorted_tuples:  # 检查排序后的元组是否非空
                comments_inputid_one, comments_mask_one, comments_like_one = (list(t) for t in zip(*sorted_tuples))
    ...
    comments_inputid = pad_sequence(num_comments, comments_inputid_resorted, 256)
    comments_mask = pad_sequence(num_comments, comments_mask_resorted, 256)
    comments_like = []
...
class Run():
    def __init__(self,
        ...
        self.event_num = 616
        self.mode = 'normal'
        self.modality_type = config['modality_type']

    def get_dataloader(self, data_type, data_fold):
        collate_fn = None
        datasets = []
        if data_type == 'FakeND_RS':

            for i in range(1, 6):
                if i != data_fold:
                   dataset = FakeND_RSDataset(f'vid_fold_{i}.txt', self.modality_type, 'train')
                   datasets.append(dataset)
            dataset_train = ConcatDataset(datasets)

            dataset_test = FakeND_RSDataset(f'vid_fold_{data_fold}.txt', self.modality_type, 'test')
            collate_fn = FakeND_RS_collate_fn
        elif data_type == 'FANVM':
            dataset_train = FANVMDataset_train(f'vid_fold_{data_fold}.txt')
            dataset_test = FANVMDataset_test(path_vid_train=f'vid_fold_{data_fold}.txt',
                                             path_vid_test=f'vid_fold_{data_fold}.txt')
            collate_fn = FANVM_collate_fn
        elif data_type == 'c3d':
            dataset_train = C3DDataset(f'vid_fold_{data_fold}.txt')
            dataset_test = C3DDataset(f'vid_fold_{data_fold}.txt')
            collate_fn = c3d_collate_fn
        elif data_type == 'vgg':
            dataset_train = VGGDataset(f'vid_fold_{data_fold}.txt')
            dataset_test = VGGDataset(f'vid_fold_{data_fold}.txt')
            collate_fn = vgg_collate_fn
        elif data_type == 'bbox':
            dataset_train = BboxDataset('vid_fold_1.txt')
    ...
    def get_dataloader_temporal(self, data_type):
        collate_fn = None
        if data_type == 'FakeND_RS':
            dataset_train = FakeND_RSDataset('vid_time3_train.txt')
            dataset_val = FakeND_RSDataset('vid_time3_val.txt')
            dataset_test = FakeND_RSDataset('vid_time3_test.txt')
            collate_fn = FakeND_RS_collate_fn
        elif data_type == 'FANVM':
    ...
    def get_model(self):
        if self.model_name == 'FakeND_RS':
            self.model = FakeND_RSModel(bert_model='bert-base-chinese', fea_dim=128, dropout=self.dropout)
    ...
    def main(self):
        if self.mode_eval == "nocv":
            ...
            for fold in range(1, 6):
                ...
                wandb.init(project='FakeSV', name=f"Fold_{fold}_{self.modality_type}",
                           config={"model_name": "FakeND_RS", "mode_eval": 'cv', "epoches": 30, "batch_size": 128,
                                   "num_workers": 0, "epoch_stop": 5,
                                   "seed": 2022, "gpu": True, 'lr': 0.0001,
                                   "lambd": 0.1, "dropout": 0.1, "weight_decay": 5e-5,
                                   "path_param": './checkpoints/',
                                   'path_tensorboard': './tb/'})
