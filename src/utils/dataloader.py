...
class FakeND_RSDataset(Dataset):

    def __init__(self, path_vid, modality_type, train_or_test, datamode='title+ocr'):

        with open('/path/to/file', "rb") as fr:
            self.dict_vid_convfea = pickle.load(fr)

        self.data_complete = pd.read_json('/path/to/file',orient='records',dtype=False,lines=True)
        self.data_complete = self.data_complete[self.data_complete['annotation']!=2] # label: 0-real, 1-fake, 2-debunk

        self.framefeapath='/path/to/file'
        self.c3dfeapath='/path/to/file'



        self.vid = []

        with open('/path/to/file'+ path_vid, "r") as fr:
             for line in fr.readlines():
                 self.vid.append(line.strip())

        self.c3d = h5py.File(self.c3dfeapath, "r")
        self.frame = pickle.load(open(self.framefeapath, 'rb'))
        ...
        self.tokenizer = BertTokenizer.from_pretrained('/path/to/file')
    ...
    def __getitem__(self, idx):
        ...
        title_tokens['input_ids'] = [0 if x is None else x for x in title_tokens['input_ids']]
        ...
        comments_inputid = torch.LongTensor(np.array(comments_inputid).astype(float).astype(int))
        comments_mask = torch.LongTensor(np.array(comments_mask))

        comments_like = []
        for num in [item['count_like']]:
            if isinstance(num, str):
               num_like = num.split(" ")[0]
               comments_like.append(str2num(num_like))
            else:
               comments_like.append(num)
        comments_like = torch.tensor(comments_like)
        

        if vid in self.dict_vid_convfea:
            audioframes = self.dict_vid_convfea[vid]

        else:
            audioframes = torch.zeros([15, 12288], dtype=torch.float)

        audioframes = torch.FloatTensor(audioframes)
        

        frames = self.frame[vid+'.pkl']
        frames=torch.FloatTensor(frames)



        c3d = self.c3d[vid]['c3d_features']
        ...
        if any(x is None for x in intro_tokens['input_ids']):

            intro_tokens['input_ids'] = [0 if x is None else x for x in intro_tokens['input_ids']]
